...
export function buildPdfReport(
	...
	const totalIncome = data.entries.reduce(
	...
	doc.text(formatCurrency(totalIncome), pageW - PAGE_MARGIN - 40, summaryY, {
		align: "right",
	});
	summaryY += 7;
	doc.text("Total Expenses:", PAGE_MARGIN, summaryY);
	doc.text(formatCurrency(totalExpenses), pageW - PAGE_MARGIN - 40, summaryY, {
		align: "right",
	});
	summaryY += 7;
	...
	if (receiptsWithImages.length > 0) {
		...
		receiptsWithImages.forEach((row, index) => {
			...
			doc.setFontSize(8);
			doc.setFont("helvetica", "bold");
			doc.text(caption, cellX + 3, cellY + 5.5, {
			...
			const footerText =
				row.receipt.submitterName + " — " + row.receipt.purpose;
			doc.setFontSize(7);
...
// ----- Account ledger (treasury per account) -----

export type AccountLedgerPdfEntry = {
	date: Date;
	description: string;
	amount: number;
	statusLabel: string;
	linkedCashflowDescription?: string | null;
};

export type AccountLedgerPdfInput = {
	account: string;
	entries: AccountLedgerPdfEntry[];
	startingBalance: number | null;
	endingBalance: number | null;
};

function sortLedgerEntriesAsc(
	entries: AccountLedgerPdfEntry[],
): AccountLedgerPdfEntry[] {
	return [...entries].sort(
		(a, b) =>
...
/**
 * Tabular ledger for one treasury account (inflow / outflow / running balance).
 * Entries should be pre-filtered; they are sorted ascending by date for running totals.
 */
export function buildAccountLedgerPdf(
	data: AccountLedgerPdfInput,
	options: { dateFrom?: string; dateTo?: string; title?: string } = {},
): jsPDF {
	const doc = new jsPDF({ orientation: "portrait", unit: "mm", format: "a4" });
	const pageW = doc.getPageWidth();

	const sorted = sortLedgerEntriesAsc(data.entries);
	let running = data.startingBalance ?? 0;

	const tableBody: string[][] = [];
	for (const e of sorted) {
		running += e.amount;
		const desc = e.linkedCashflowDescription?.trim()
			? `${e.description}\nVerified as: ${e.linkedCashflowDescription.trim()}`
			: e.description;
		tableBody.push([
			new Date(e.date).toLocaleDateString("en-PH"),
			desc,
	...
	const totalInflow = sorted.reduce(
		(s, e) => s + (e.amount > 0 ? e.amount : 0),
		0,
	);
	const totalOutflow = sorted.reduce(
		(s, e) => s + (e.amount < 0 ? Math.abs(e.amount) : 0),
		0,
	);
	...
	doc.setFontSize(18);
	doc.text(
		options.title ?? `Account ledger — ${data.account}`,
		PAGE_MARGIN,
		18,
	);
	doc.setFontSize(10);
	const period =
		options.dateFrom && options.dateTo
			? `${options.dateFrom} to ${options.dateTo}`
			: "All dates";
	doc.text(`Account: ${data.account}`, PAGE_MARGIN, 25);
	doc.text(`Period: ${period}`, PAGE_MARGIN, 30);
	doc.text(`Generated: ${new Date().toLocaleString("en-PH")}`, PAGE_MARGIN, 35);
	...
	if (data.startingBalance != null) {
		...
		doc.setFont("helvetica", "bold");
		doc.text(
			`Opening balance: ${formatCurrency(data.startingBalance)}`,
			PAGE_MARGIN,
			tableStartY,
		);
		doc.setFont("helvetica", "normal");
	...
	autoTable(doc, {
		startY: tableStartY,
		head: [
			["Date", "Description", "Inflow", "Outflow", "Running balance", "Status"],
		],
		body: tableBody,
		margin: { left: PAGE_MARGIN, right: PAGE_MARGIN },
		styles: {
		...
		columnStyles: {
			...
			2: { cellWidth: 28, halign: "right", overflow: "linebreak" },
			3: { cellWidth: 28, halign: "right", overflow: "linebreak" },
			4: { cellWidth: 32, halign: "right", overflow: "linebreak" },
	...
	doc.setFontSize(10);
	doc.setFont("helvetica", "bold");
	doc.text("Total inflow:", PAGE_MARGIN, summaryY);
	doc.text(formatCurrency(totalInflow), pageW - PAGE_MARGIN - 40, summaryY, {
		align: "right",
	});
	summaryY += 7;
	doc.text("Total outflow:", PAGE_MARGIN, summaryY);
	doc.text(formatCurrency(totalOutflow), pageW - PAGE_MARGIN - 40, summaryY, {
		align: "right",
	});
	summaryY += 7;
	doc.text("Net (period):", PAGE_MARGIN, summaryY);
	const netStr =
	...
	if (data.endingBalance != null) {
		...
		doc.text(`Closing balance: ${endStr}`, PAGE_MARGIN, summaryY);
	}
	doc.setFont("helvetica", "normal");
...
export function downloadAccountLedgerPdf(
	data: AccountLedgerPdfInput,
	options: { dateFrom?: string; dateTo?: string; filename?: string } = {},
): void {
	const doc = buildAccountLedgerPdf(data, {
		...options,
		title: `Account ledger — ${data.account}`,
	});
	const safeAccount = data.account.replace(/[/\\?%*:|"<>]/g, "-");
	const from = options.dateFrom ?? "all";
	const to = options.dateTo ?? "all";
	const name =
		options.filename ??
		`ledger-${safeAccount}-${from}-${to}.pdf`.replace(/[/\\]/g, "-");
	doc.save(name);
}
...
export function downloadActivityLogPdf(
	...
	const name =
		options.filename ??
		`activity-log-${options.dateFrom ?? "all"}-${options.dateTo ?? "all"}.pdf`.replace(
			/\/|\\/g,
			"-",
		);
	doc.save(name);
...
export function buildProjectReportPdf(data: ProjectReportData): jsPDF {
	...
	const totalActualIncome = (data.incomeRows ?? []).reduce(
		(s, r) => s + r.amount,
		0,
	);
	const totalActual = (data.expenditureRows ?? []).reduce(
		(s, r) => s + r.amount,
		0,
	);
	...
	doc.text(formatCurrency(totalIncomeBudget), pageW - PAGE_MARGIN - 50, y, {
		align: "right",
	});
	y += 6;
	doc.text("  Actual Revenue", PAGE_MARGIN, y);
	doc.text(formatCurrency(totalActualIncome), pageW - PAGE_MARGIN - 50, y, {
		align: "right",
	});
	y += 6;
	...
	doc.text(formatCurrency(data.totalBudget), pageW - PAGE_MARGIN - 50, y, {
		align: "right",
	});
	y += 6;
	doc.text("  Actual Expenditures", PAGE_MARGIN, y);
	doc.text(formatCurrency(totalActual), pageW - PAGE_MARGIN - 50, y, {
		align: "right",
	});
	y += 6;
	doc.text("  Variance", PAGE_MARGIN, y);
	...
	if (projectReceiptsWithImages.length > 0) {
		doc.addPage();
		const totalReceiptPages = Math.ceil(
			projectReceiptsWithImages.length / RECEIPTS_PER_PAGE,
		);
		const cols = 2;
		const receiptStartY = 22;
...
export function downloadProjectReportPdf(
	...
	const name =
		options.filename ??
		`project-report-${safeName}-${new Date().toISOString().slice(0, 10)}.pdf`;
	doc.save(name);
}
