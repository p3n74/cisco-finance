...
import { z } from "zod";
import { type Context, WS_EVENTS, type WsEmitter } from "../context";
import {
	accountEditorProcedure,
	budgetEditorProcedure,
	cashflowEditorProcedure,
	protectedProcedure,
	publicProcedure,
	receiptEditorProcedure,
	router,
	whitelistedProcedure,
} from "../index";
...
export const appRouter = router({
	...
	activityLog: router({
		list: whitelistedProcedure
			...
			.query(async ({ ctx, input }) => {
				...
				const nextCursor =
					!hasDateRange && logs.length > effectiveLimit
						? logs[effectiveLimit - 1].id
						: null;
	...
	overview: router({
		stats: whitelistedProcedure.query(async ({ ctx }) => {
			...
			const pendingVerificationAmount = Number(
				pendingVerificationData._sum.amount ?? 0,
			);
	...
	receiptSubmission: router({
		submit: publicProcedure
			.input(
				z.object({
					...
					purpose: z
						.string()
						.min(5, "Please describe what this receipt is for"),
					imageData: z
						.string()
						.min(1, "Please upload a receipt image")
						.max(
							MAX_RECEIPT_IMAGE_BASE64_LENGTH,
		...
		list: whitelistedProcedure
			...
			.query(async ({ ctx, input }) => {
				...
				const nextCursor =
					submissions.length > limit ? submissions[limit - 1].id : null;
		...
		listPage: whitelistedProcedure
			.input(
				z.object({
					...
					statusFilter: z
						.enum(["all", "bound", "unbound"])
						.optional()
						.default("all"),
					reimbursementFilter: z
						.enum(["all", "needs", "none", "reimbursed", "endorsed"])
						.optional()
						.default("all"),
		...
		bind: receiptEditorProcedure
			...
			.mutation(async ({ ctx, input }) => {
				const [receipt, cashflow] = await Promise.all([
					...
					ctx.prisma.cashflowEntry.findUnique({
		...
		submitAndBind: receiptEditorProcedure
			...
			.mutation(async ({ ctx, input }) => {
				...
				return {
					id: submission.id,
					message: "Receipt uploaded and bound successfully",
				};
			}),
		...
		endorse: receiptEditorProcedure
			...
			.mutation(async ({ ctx, input }) => {
				if (ctx.userRole !== "AUDITOR" && ctx.userRole !== "VP_FINANCE") {
					throw new Error(
						"Only the Auditor or VP Finance can endorse receipts for reimbursement.",
					);
				}
				...
				if (!treasurerAuth?.email) {
					throw new Error(
						"Treasurer email not found. Please ensure a treasurer is configured.",
					);
				}
		...
		markAsReimbursed: receiptEditorProcedure
			...
			.mutation(async ({ ctx, input }) => {
				...
				if (!submission.endorsedAt) {
					throw new Error(
						"This receipt has not been endorsed yet. It must be endorsed by the Auditor or VP Finance first.",
					);
				}
	...
	accountEntries: router({
			...
			.mutation(async ({ ctx, input }) => {
				...
				if (entry?.cashflowEntry) {
					throw new TRPCError({
						code: "BAD_REQUEST",
						message:
							"Cannot archive: this entry is linked to a verified transaction.",
					});
				}
		...
		// Aggregate summary for accounts page stats (all active entries, no pagination)
		summary: whitelistedProcedure.query(async ({ ctx }) => {
			const [
				totalAgg,
				inflowAgg,
				outflowAgg,
				...
				ctx.prisma.accountEntry.aggregate({
					where: { isActive: true, account: "GCash" },
					_sum: { amount: true },
				}),
				ctx.prisma.accountEntry.aggregate({
					where: { isActive: true, account: "GoTyme" },
					_sum: { amount: true },
				}),
				ctx.prisma.accountEntry.aggregate({
					where: { isActive: true, account: "Cash" },
					_sum: { amount: true },
				}),
				ctx.prisma.accountEntry.aggregate({
					where: { isActive: true, account: "BPI" },
					_sum: { amount: true },
				}),
			]);

			const netMovement = Number(totalAgg._sum.amount ?? 0);
			const totalInflow = Number(inflowAgg._sum.amount ?? 0);
			const totalOutflow = Math.abs(Number(outflowAgg._sum.amount ?? 0));
			...
			return {
				...
				accountBalances: {
					GCash: Number(gcashAgg._sum.amount ?? 0),
					GoTyme: Number(gotymeAgg._sum.amount ?? 0),
					Cash: Number(cashAgg._sum.amount ?? 0),
					BPI: Number(bpiAgg._sum.amount ?? 0),
				},
			};
		}),
	}),
						...
						const totalActual = project.items.reduce(
							(sum, item) =>
								...
								item.expenses.reduce(
									(expSum, exp) =>
										expSum + Math.abs(Number(exp.cashflowEntry.amount)),
									0,
								),
							0,
						);
							...
							items: project.items.map((item) => ({
								...
								actualAmount:
									item.type === "expense"
										? item.expenses.reduce(
												(sum, exp) =>
													sum + Math.abs(Number(exp.cashflowEntry.amount)),
												0,
											)
										: item.incomes.reduce(
					...
					items: project.items.map((item) => ({
						...
						expenses: item.expenses.map((exp) => ({
							...
							cashflowEntry: {
								...
								amount: Number(exp.cashflowEntry.amount),
								lineItems:
									exp.cashflowEntry.lineItems?.map((li) => ({
										id: li.id,
										description: li.description,
										category: li.category,
										amount: Number(li.amount),
						...
						incomes: item.incomes.map((inc) => ({
							...
							cashflowEntry: {
								...
								amount: Number(inc.cashflowEntry.amount),
								lineItems:
									inc.cashflowEntry.lineItems?.map((li) => ({
										id: li.id,
										description: li.description,
										category: li.category,
										amount: Number(li.amount),
				...
				(sum, p) =>
					...
					p.items.reduce(
						(iSum, item) =>
							...
							item.expenses.reduce(
								(eSum, exp) =>
									eSum + Math.abs(Number(exp.cashflowEntry.amount)),
								0,
							),
						0,
					),
	...
	budgetItems: router({
		...
		linkIncome: budgetEditorProcedure
			...
			.mutation(async ({ ctx, input }) => {
				...
				if (item.type !== "income") {
					throw new Error(
						"Only income-type budget items can link income. Change item type to Income first.",
					);
				}
				...
				if (Number(cashflow.amount) <= 0) {
					throw new Error(
						"Only positive (inflow) cashflow entries can be linked as income.",
					);
				}
	...
	cashflowEntries: router({
		...
		listPage: whitelistedProcedure
			.input(
				z.object({
					...
					search: z.string().optional(),
					statusFilter: z
						.enum(["all", "no_receipt", "verified", "manual"])
						.optional()
						.default("all"),
					dateFrom: z.string().optional(),
					dateTo: z.string().optional(),
			...
			.query(async ({ ctx, input }) => {
				const whereParts: Parameters<
					typeof ctx.prisma.cashflowEntry.findMany
				>[0]["where"] & { AND: unknown[] } = {
					isActive: true,
					AND: [],
				};
		...
		// Aggregate summary for dashboard stats (all active entries, no pagination)
		summary: whitelistedProcedure.query(async ({ ctx }) => {
			const [totalAgg, inflowAgg, outflowAgg] = await Promise.all([
				ctx.prisma.cashflowEntry.aggregate({
					where: { isActive: true },
					_sum: { amount: true },
					_count: true,
			...
			const netCashflow = Number(totalAgg._sum.amount ?? 0);
			const totalInflow = Number(inflowAgg._sum.amount ?? 0);
			const totalOutflow = Math.abs(Number(outflowAgg._sum.amount ?? 0));
		...
		getReportData: whitelistedProcedure
			...
			.query(async ({ ctx, input }) => {
				...
				if (input.dateFrom && input.dateTo) {
					...
					const netInPeriod = entries.reduce(
